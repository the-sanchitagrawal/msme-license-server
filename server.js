...
// ── GET /admin/backup — download full db.json ──────────────────
app.get('/admin/backup', requireAdmin, (req, res) => {
  const db = readDB();
  res.setHeader('Content-Disposition', `attachment; filename=msme-db-backup-${new Date().toISOString().slice(0,10)}.json`);
  res.setHeader('Content-Type', 'application/json');
  res.send(JSON.stringify(db, null, 2));
});

// ── POST /admin/restore — restore from uploaded db.json ────────
app.post('/admin/restore', requireAdmin, (req, res) => {
  try {
    const { licenses, usage_log } = req.body;
    if (!licenses) return res.status(400).json({ error: 'Invalid backup file — missing licenses.' });
    writeDB({ licenses, usage_log: usage_log || [] });
    res.json({ success: true, licensesRestored: Object.keys(licenses).length });
